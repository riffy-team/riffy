...
client.on('ready', () => {
    ...
    console.log('Bot is ready with failover enabled!');
});
...
riffy.on('nodeDisconnect', (node) => {
    console.log(`Node ${node.name} disconnected. Players will be migrated automatically.`);
});

riffy.on('playerMigrated', (player, oldNode, newNode) => {
    console.log(`Player migrated from ${oldNode.name} to ${newNode.name} for guild ${player.guildId}`);
});
